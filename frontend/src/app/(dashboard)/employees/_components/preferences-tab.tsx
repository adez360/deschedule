...
export function PreferencesTab({
  ...
  const { data: prefData, isLoading } = useQuery({
    ...
    queryFn: () => fetchUserPreferences(userId, token),
  ...
  const saveMut = useMutation({
    mutationFn: async () => {
      ...
      await saveUserPreferences(userId, enabled.map((s) => ({ store_id: s.id, weight: s.weight })), token);
      const cap = dailyMax === "" ? null : Math.max(1, Math.min(24, parseInt(dailyMax, 10)));
      if (cap !== (dailyHourMax ?? null)) await updateUser(userId, { daily_hour_max: cap }, token);
    },
    onSuccess: () => {
