...
### 3.7.1 門市能力需求（StoreSkillDemand）🔄 已修訂（IDEA-02，2026-06-08）
...
> 🔄 **IDEA-02 修訂（2026-06-08）— 取代原「方案 A：疊加層 + 子需求人數」設計（方案 A 曾上線，見議題 9 → 議題 10）**：
...
- [ ] 🔄 **工作能力需求改版（IDEA-02，2026-06-08）**：能力改純標籤（`StoreSkillDemand.slots` `int[7][24]` → `boolean[7][24]`）、與人數需求單表整合（同一筆刷式「選取範圍 → 點選」操作）、移除疊加層與子網格編輯器、演算法子配額由「N 人」改為「至少 1 人」覆蓋約束。含 Alembic migration、後端 schema/scheduler 調整、`/settings/demand` 與 `skills-api.ts` 前端重構（見 3.7.1、4.2、議題 10）
...
10. ~~**工作能力需求的設定方式與資料模型（IDEA-02）**~~ ✅ **已決策（2026-06-08，IDEA-02）— 修訂議題 9 的方案 A**：
    ...
    - **待實作**（見第 10 節路線圖 Phase 3「工作能力需求改版」）。詳見 3.7.1、4.2。

---

*文件版本：v0.9 — 2026-06-08*
*新增 IDEA-02 修訂（議題 10）：工作能力需求由「方案 A 疊加層 + 子需求人數（int）」改版為「標籤制（boolean）+ 與人數需求單表整合」。能力不再設人數、與人數採相同的「選取範圍 → 點選筆刷」操作、單表同時顯示、單一儲存動作一併送出；排班演算法子配額由「N 人」改為「至少 1 人」覆蓋約束。待實作（Phase 3「工作能力需求改版」）。詳見 3.7.1、4.2、議題 9→10。*
