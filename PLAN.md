...
```
EmployeeContract（v3 — 已決策並完成實作，2026-06-09，移除 store_id，合約改為組織層級跨門市）
├── id: UUID
├── user_id: UUID（FK）
...
> ✅ **已決策並完成實作（2026-06-09）**：合約從「per-store（每門市獨立一份）」改為「org-level（跨門市、每員工一份有效合約）」，移除 `store_id` 欄位與 FK 限制。**理由**：合約（FT/PT 類型、月薪/時薪）是員工在組織層級的基本勞動條件，與員工在哪間門市值班無關；原設計要求管理者為每間門市分別設定合約，造成冗餘與不一致。**影響範圍**：Alembic migration `e1f2a3b4c5d6` 移除 `store_id` 欄位；`EmployeeContract` model 移除 `store_id`/`store` 關聯；`ContractResponse` schema 移除 `store_id`；contracts router 改為 `GET/PUT /users/{user_id}/contract`（跨門市）；`_create_payroll_reports` 的合約查詢移除 `store_id` filter（`contract_map` 改以 `user_id` 為鍵）；`PayrollReport` 的 `store_id` 不變（薪資報表仍以門市為單位）。
...
### 3.12 薪資加減項（PayrollAdjustment）🆕 已實作（2026-06-11，IDEA-06）

「其他項目」——員工每月的手動加減項（加班 +、交通 +、自訂 ±…），與底薪一同累加為當月總計。以 (員工, 年, 月) 為單位，不分門市（對應個人月報表）。
...
├── year: int
├── month: int
├── label: string（項目名稱，例：加班 / 交通 / 自訂）
├── amount: decimal（帶正負號，+ 為加項、− 為減項）
├── currency: string（預設 TWD）
├── created_at / updated_at: timestamp
```

> ✅ **已實作（2026-06-11）**：Alembic migration `c3d4e5f6a7b8`。API：`GET /users/{userId}/payroll-adjustments?year=&month=`（自己可看自己，否則需 `employee.payroll.view`）、`POST /users/{userId}/payroll-adjustments`、`PATCH/DELETE /payroll-adjustments/{id}`（編輯一律需 `employee.payroll.view`）。前端於 `/payroll`「個人」報表頁內以行內表格新增/編輯/刪除。
...
**IDEAS-03 改版（2026-06-09，已完成並通過瀏覽器端到端驗證）**：根據使用者回饋「拖曳範圍鎖定單日欄不夠直覺，希望可以跨多天一次框選」，互動模型再次重新設計：

- **跨日多時段框選**：移除單日欄鎖定（`d !== drag.current.day` 攔截），改為與 `/settings/demand` 一致的全域矩形框選（`{ dMin, dMax, rMin, rMax }`），支援橫跨多日多小時的連續範圍
- **互動模型：先框選 → 再操作**：
  - **指派**：框選完成後，在左側員工清單點選員工 → 立即填入選取範圍所有格子（取代原工具列員工下拉選單 + 「指派整段班次」按鈕）
  - **清除**：框選完成後，點工具列的「清除此範圍排班」按鈕 → 行內二次確認後批次刪除（邏輯不變）
- **員工清單可用性標記**：框選有效時，側邊欄每位員工自動顯示對選取範圍的可用性（✓ 全段可用 / ⚠ 部分可用 / ✗ 未標記），透過 `useQueries` 並聯呼叫各員工的可用時段 API 取得，結果由 TanStack Query 快取（`staleTime: 5min`）；員工卡片在框選時高亮為可點擊狀態（`border-indigo-500/40`），無框選時還原為唯讀
- **移除 Undo**：操作有明確二次確認（清除時確認、指派時可看到可用性提示），不需要撤銷機制；移除 `undoStack`、`pushUndo`、`handleUndo`、`UndoOp` type、`<Undo2>` 按鈕

技術變更：`Selection` 型別由 `{ day, rMin, rMax }` 改為 `{ dMin, dMax, rMin, rMax }`；`DragState` 由 `{ active, day, origin, end }` 改為 `{ active, dOrigin, rOrigin, dCur, rCur }`；新增 `AvailStatus = "full" | "partial" | "none" | "loading"` 型別；`useQueries` 並聯查詢所有員工可用性；`handleAssign(userId)` 改為接受員工 ID 參數（不再從 `pickedEmployeeId` state 讀取）
...
> ✅ **IDEA-06 改版完成並通過瀏覽器端到端驗證（2026-06-11）**：`/payroll` 重構為「個人 / 門市」雙視圖（Tabs），權限分流，並新增「其他項目」加減項與 FT 所屬門市歸屬。

**權限與視圖**：
- `canManage`（持 `employee.payroll.view` 或 `system.all`）：可見「門市」+「個人」兩個分頁；「個人」分頁左側有員工選擇面板，可檢視任一員工並編輯其加減項。
- 一般員工：只見「個人」分頁，鎖定為本人、唯讀（不可編輯加減項）。透過 `GET /users/me/payroll`（免特殊權限）取得本人資料。側邊欄「薪資報表」移至「個人」群組對所有人顯示。

**「門市」報表（管理視圖，依門市分區）**：
- 門市選擇（全部 / 單一）+ 月份導覽 + 匯出 CSV + 重新計算（單一門市時）
- 每個門市一個區塊：員工 | 合約 | 時數 | 薪資 + 小計
- 薪資依合約分流：PT＝`total_hours × hourly_rate`（跨週加總）；FT＝合約 `monthly_salary`，**僅在該員工所屬門市（`home_store_id`）的區塊顯示一次**，其他門市區塊顯示「—（所屬門市名）」；CUSTOM＝「—」

**「個人」報表（單一員工月報表）**：
1. **基本薪資**：門市 | 時數 | 薪資（逐門市）+ 小計。FT 月薪只計入所屬門市列（標「所屬」徽章）；若所屬門市當月無班，補一筆 0 工時的所屬門市列以正確歸屬月薪；未設定所屬門市則提示警告。
2. **其他項目**（見 3.12 `PayrollAdjustment`）：項目 | 金額（帶正負）+ 小計；管理者可行內新增/編輯/刪除。
3. **總計薪資** = 基本薪資小計 + 其他項目小計。

- **匯出格式**：CSV（門市視圖，依當前篩選輸出每筆 `PayrollReport`）；PDF（列印用，尚未實作）
...
> **✅ 已實作（2026-06-17，IDEA-14）**：作為 `/schedules` 員工入口（無 `store.schedule.edit` 等管理權限者進場），標題「我的班表」。`_components/employee-schedule.tsx` 提供兩種模式（B1）：「週曆」（預設，rows=門市、cols=週一～日，依 `Store.color` 分色，缺色退回色盤）＋「圖表」（時×日格、依門市色填滿）；跨門市彙整「自己」當週班次（C1）、僅含已發佈/已封存（員工看不到草稿）、保留 iCal 訂閱與全螢幕。
...
> **進度（2026-06-16）**：清單 + CRUD 已實作；管理介面已擴充門市負責人（`Store.manager_user_id`）與門市代表色（`Store.color`，migration `c5d6e7f8a9b0`）。
> **✅ item 3 已實作（2026-06-17，IDEA-14）**：併入「班表檢視雙入口」一併處理。`/schedules` 改為依權限分流（A1）：`store.schedule.edit`／`org.schedule.arrange`／`org.schedule.view_all`／`system.all` → 管理版（原有員工 Grid 已是 D1「不分身份組列出所有有班員工」+ 產生/發布/封存/手動編輯 + 覆蓋率分頁新增 E1 能力需求疊加角標）；其餘登入者 → 員工唯讀版（見 5.3.2）。能力疊加 E1：覆蓋率格底色不變，缺能力時段加 `AlertTriangle` 角標 + tooltip 列出缺哪些能力（前端用 `fetchSkillDemand` + 對當週有班員工批次 `fetchUserSkills` 現算，無新後端端點；獨立於人數需求判定）。詳見 `ideas/idea-14-store-schedule-view.md`。`Store.manager_user_id` 本次不連動（G3）。
...
# 員工合約（upsert 自動結算舊合約；v3 改為跨門市 org-level）
GET    /api/users/:userId/contract          ← 取得目前有效合約（跨門市）
PUT    /api/users/:userId/contract          ← 新增或更新有效合約（跨門市）
GET    /api/users/:userId/contracts         ← 合約歷史紀錄（所有期間）
...
- [x] 手動排班 v1 —— 已完成並通過瀏覽器端到端驗證（2026-06-08），詳見 5.1.2
- [x] 🆕 手動排班 IDEAS-03 改版 —— 跨日框選 + 左側員工清單可用性標記 + 移除 Undo（2026-06-09，詳見 5.1.2）
- [x] 熱力圖覆蓋層 —— `CoverageHeatmap` 改接真實 `DemandTemplate` API，移除 hardcoded mock（2026-06-09）
- [x] 歷史班表瀏覽 —— 新增「歷史」分頁，週次清單最新在前，一鍵跳轉 + 自動切員工視角，封存班表鎖定手動編輯 tab（2026-06-09）
...
- [x] 🆕 **門市管理頁面（5.3.3）**：清單 + CRUD 已完成（卡片清單、搜尋、新增/編輯/刪除 dialog）。**管理介面擴充（2026-06-16）**：新增 `Store.manager_user_id`（門市負責人，FK users，`ON DELETE SET NULL`）+ `Store.color`（門市代表色，hex）。含 Alembic migration `c5d6e7f8a9b0`、`schemas/store.py` + `stores.py` PATCH（兩欄可清空）、`/settings/stores` dialog 加負責人下拉 + 8 色代表色選擇器、卡片顯示色彩 accent + 負責人。**item 3 門市班表檢視已於 IDEA-14 完成（見下）。**
- [x] 🆕 **班表檢視雙入口（IDEA-14）**（2026-06-17）：`/schedules` 依權限分流（A1，同一路由）。**管理版**（`store.schedule.edit`／`org.schedule.arrange`／`org.schedule.view_all`／`system.all`）：原有員工 Grid（D1，不分身份組列出所有有班員工）+ 產生/發布/封存/手動編輯，覆蓋率分頁新增能力需求疊加（E1：缺能力時段 `AlertTriangle` 角標 + tooltip，前端用 `fetchSkillDemand` + 批次 `fetchUserSkills` 現算，無新後端端點）。**員工版**（其餘登入者，標題「我的班表」）：新 `_components/employee-schedule.tsx`，週曆（預設，依 `Store.color` 分色）+ 圖表雙模式（B1），跨門市彙整自己當週班次（C1）、僅含已發佈/已封存、保留 iCal 訂閱 + 全螢幕。`Store.manager_user_id` 本次不連動（G3）。無 migration（純前端 + 既有 org-access 讀取端點）。見 5.3.2、5.3.3 item 3、ideas/idea-14-store-schedule-view.md
