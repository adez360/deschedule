...
### 3.7.1 門市能力需求（StoreSkillDemand）🆕 提案中

來源：IDEA-01「門市管理頁面 — 工作能力需求表」。標示哪些時段需要哪些能力（見 3.4.3 `Skill`）的員工才能正常運作，供自動排班演算法作為額外限制條件。
...
├── slots: int[7][24]（各時段需求「具備此能力」的人數）
└── updated_at: timestamp
```

> IDEA-01 提到「人力需求表與能力需求表若能合併設計會更好，若實作複雜或 UI/UX 不佳則拆成兩表」。本提案先以獨立表設計（與 `DemandTemplate` 分離），原因：
> 1. 大多數時段不需特定能力，獨立表可避免 `DemandTemplate.slots` 維度爆炸（int → int+能力矩陣）
> 2. 排班演算法可將「總人數需求」與「特定能力需求」拆開驗證，邏輯更單純
> 3. UI 上可用疊加圖層（在需求熱力圖上疊加能力標記）呈現，不需重新設計主表格
> 若後續發現雙表維護成本過高，可再評估改為 `DemandTemplate.slots: { count: int, required_skills: UUID[] }[7][24]` 的合併結構。
...
- [ ] 完整稽核日誌頁面（含鎖定後修改記錄）
- [ ] 效能調校與壓力測試
- [ ] 🆕 職位等級／工作能力系統（`Skill` / `UserSkill` / `StoreSkillDemand`，見 3.4.3、3.7.1，排班演算法納入能力限制）
...
9. 🆕 **能力需求表與人力需求表是否合併**（來源：IDEA-01）：見 3.7.1 說明，初步建議先以獨立的 `StoreSkillDemand` 表實作，待 UI/UX 驗證後再評估是否合併進 `DemandTemplate.slots`。

---

*文件版本：v0.7 — 2026-06-08*
*Phase 1 全部完成。議題 2、3、4、5、6、7、8 已決策；議題 9 待決策（源自 IDEA-01 提案）。API 路由已更新為實際實作版本。新增：JWT 過期自動登出（SessionGuard）、身份組適用範圍多選（store_ids）；整合 IDEA-01 臨時提案（員工管理頁面、個人資料擴充、門市管理頁面、職位能力系統、AI 輔助審查）。**議題 7 已決策並完成實作（2026-06-08）：合約模型重新設計（FT 月薪 / PT 時薪 / CUSTOM 無薪，移除起訖時間與工時上下限），且自動排班不設定每週工時上限**——已通過瀏覽器端到端驗證。**議題 8 已決策（2026-06-08）：新增權限位 `employee.identity.view`，透過身份組授予；持有者可見真實姓名 `name`，否則僅見 `nickname`**——詳見 3.1、第 2 節權限表、第 11 節議題 8。*
