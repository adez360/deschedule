...
class StoreSkillDemand(Base):
    """Standing per-skill required coverage for a store (IDEA-15). One row per
    (store, skill) — no week dimension. slots: bool[7][24] — index 0 = Monday 00:00."""
    __tablename__ = "store_skill_demands"
    __table_args__ = (
        UniqueConstraint("store_id", "skill_id", name="uq_skill_demand_store_skill"),
    )
